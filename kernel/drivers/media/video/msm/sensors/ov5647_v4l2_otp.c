...
/*           |                      |                      |preview blue      */
/* 04/03/2013|Zhang Lu	            |CR-399266             |second source     */
/* ----------|----------------------|----------------------|----------------- */
/******************************************************************************/
#include <linux/i2c.h>
...
#define OV5647_TRULY_RG_RATIO_TYPICAL_VALUE 0x43 /*truly R/G of typical camera module */
#define OV5647_TRULY_BG_RATIO_TYPICAL_VALUE 0x66 /*truly B/G of typical camera module */
#define GAIN_RH_ADDR	0x5186
#define GAIN_RL_ADDR	0x5187
#define GAIN_GH_ADDR	0x5188
#define GAIN_GL_ADDR	0x5189
...
int read_otp(int index, struct otp_struct * otp_ptr)
{
	...
	ov5647_customer_id = (*otp_ptr).customer_id;
#endif
	// disable otp read
	//OV5647_write_i2c(0x3d21, 0x00);
	ov5647_i2c_write_b_sensor(0x3d21, 0x00);
...
int ov5647_v4l2_update_otp(struct msm_sensor_ctrl_t *ov5647_s_ctrl, u8 golden_rg, u8 golden_bg)
{
	...
	if(ov5647_customer_id == 0x02)
	{
		golden_rg = OV5647_TRULY_RG_RATIO_TYPICAL_VALUE;
		golden_bg = OV5647_TRULY_BG_RATIO_TYPICAL_VALUE;
	}
	if(1)//(current_otp.light_rg==0)
    {
#if OTP_DEBUG_ON        
