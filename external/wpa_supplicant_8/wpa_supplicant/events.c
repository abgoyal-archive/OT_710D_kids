...
//[BUGFIX]-Add-BEGIN by TCTNB.Fan.He,02/01/2013,PR-393597
		...
		if (!wpas_is_connect_from_framework(wpa_s)){
			if (wpas_p2p_is_connected(wpa_s) &&
			bss->freq != wpas_p2p_frequency(wpa_s)) {
			wpa_dbg(wpa_s, MSG_DEBUG, "   skip - FR200 frequency mismatch with P2P");
			continue;
			}
		}
#endif
...
//[BUGFIX]-Add-BEGIN by TCTNB.Fan.He,02/01/2013,PR-393597
#ifdef DISCONNECT_P2P_IF_USER_CONNECTS_DIFFERENT_CHANNEL_AP
		wpas_connect_from_framework(wpa_s, 0);
		wpa_printf(MSG_DEBUG, "FR200 PBC overlap");
#endif
//[BUGFIX]-Add-END by TCTNB.Fan.He
#ifdef CONFIG_P2P
...
#ifdef DISCONNECT_P2P_IF_USER_CONNECTS_DIFFERENT_CHANNEL_AP
	if(wpas_p2p_is_connected(wpa_s) &&
		selected->freq != wpas_p2p_frequency(wpa_s)){
		wpas_p2p_disconnect(wpas_p2p_get_group(wpa_s));
		wpa_dbg(wpa_s, MSG_DEBUG, " FR200 remove P2P group due to STA connection from framework %s, %d",selected->ssid, selected->freq);
	}
#endif
//[BUGFIX]-Add-END by TCTNB.Fan.He
	/*
...
//[BUGFIX]-Add-BEGIN by TCTNB.Fan.He,02/01/2013,PR-393597
#ifdef DISCONNECT_P2P_IF_USER_CONNECTS_DIFFERENT_CHANNEL_AP
		wpa_printf(MSG_DEBUG, "FR200 selected ssid %s", selected->ssid);
#endif
//[BUGFIX]-Add-END by TCTNB.Fan.He
		int skip;
