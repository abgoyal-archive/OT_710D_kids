...
)
{
	...
	u32 length;
		...
		wpa_printf(MSG_ERROR, "*****************HANDLE_QMI_EAP_REPLY CALLBACK STARTED********************* ");
...
static  Boolean eap_proxy_build_NAK(eap_proxy_sm  *eap_proxy,									u8 id, struct eap_sm *eap_sm)
{
					...
					u8 *eapReqData, int eapReqDataLen, struct eap_sm *eap_sm)
{
	 ...
	 */
			...
			wpa_printf(MSG_ERROR, "CANCELING_TIMEOUT IN EAP_CODE_SUCCESS\n");
			eloop_cancel_timeout(handle_qmi_resp_timeout, NULL, eap_proxy);
			break;
...
static Boolean eap_proxy_build_identity(eap_proxy_sm *eap_proxy, u8 id, struct eap_sm *eap_sm)
{
	...
	if (config->identity_len && config->identity != NULL) {
		...
		if (!idx && (config->identity_len == 1)) {
		...
		} else if ((idx < config->identity_len) && (config->identity != NULL)) {

			eap_auth_start.user_id_len = config->identity_len;

			eap_auth_start.user_id = os_malloc(config->identity_len);
			if (eap_auth_start.user_id != NULL) {
				os_memcpy(eap_auth_start.user_id ,
						config->identity ,
						config->identity_len);
			}
			...
			identity_format = EAP_IDENTITY_CFG_REALM;
			eap_auth_start.params_mask |= QMI_EAP_AUTH_START_EAP_USER_ID_PARAM;
			wpa_printf(MSG_ERROR, "EAP_IDENTITY_CFG_REALM selected %d \n", eap_auth_start.user_id_len);
		}
	} else {
		...
		if (config->anonymous_identity_len && config->anonymous_identity != NULL) {
			...
			identity_format = EAP_IDENTITY_IMSI_3GPP_REALM;
			eap_auth_start.params_mask |= QMI_EAP_AUTH_START_EAP_USER_ID_PARAM;
			wpa_printf(MSG_ERROR, "EAP_IDENTITY_IMSI_3GPP_REALM selected %d \n", eap_auth_start.user_id_len);
		}
	}
	...
	if (imsi == NULL) {
		...
		if (eap_proxy->eap_type == EAP_TYPE_SIM) {
			...
			os_memcpy(identity+1 , imsi , imsi_len_g);
	}
		identity_len = imsi_len_g+1;
			if (identity_format == EAP_IDENTITY_IMSI_3GPP_REALM) {
				identity_len = imsi_append_3gpp_realm(&identity, imsi_len_g+1);
