...
/* ----------|----------------------|----------------------|----------------- */
/* 02/01/2013|Fan.He                |PR-393597             |Disconnect P2P wh */
/*           |                      |                      |en connect WiFi A */
/*           |                      |                      |P.                */
/* ----------|----------------------|----------------------|----------------- */
/******************************************************************************/
...
//[BUGFIX]-Add-BEGIN by TCTNB.Fan.He,02/01/2013,PR-393597
#ifdef DISCONNECT_P2P_IF_USER_CONNECTS_DIFFERENT_CHANNEL_AP
Boolean wpas_p2p_is_connected(struct wpa_supplicant *wpa_s)
{
    if (wpas_p2p_get_group(wpa_s))
        return TRUE;
    else
	    return FALSE;
}

int wpas_p2p_frequency(struct wpa_supplicant *wpa_s)
{
    wpa_s = wpas_p2p_get_group(wpa_s);
    ...
    if (ssid->mode == WPAS_MODE_P2P_GO)
        return wpa_s->current_ssid->frequency;
    else
        return wpa_s->current_bss ? wpa_s->current_bss->freq : (int)wpa_s->assoc_freq;
}

Boolean wpas_is_connect_from_framework(struct wpa_supplicant *wpa_s)
{
    return wpa_s->connect_from_upper ? TRUE:FALSE;
}

void wpas_connect_from_framework_timeout(void *eloop_ctx, void *timeout_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;
	wpa_printf(MSG_DEBUG, "FR200 connect from framework timed out");
    wpa_s->connect_from_upper = 0;

}

void wpas_connect_from_framework(struct wpa_supplicant *wpa_s, int set)
{
    wpa_s->connect_from_upper = set;
    eloop_cancel_timeout(wpas_connect_from_framework_timeout, wpa_s, NULL);

    if(set)
        eloop_register_timeout(set, 0, wpas_connect_from_framework_timeout, wpa_s, NULL);
}

struct wpa_supplicant *wpas_p2p_get_group(struct wpa_supplicant *wpa_s)
{
	for (wpa_s = wpa_s->global->ifaces; wpa_s; wpa_s = wpa_s->next) {
		struct wpa_ssid *ssid = wpa_s->current_ssid;
		if (wpa_s->wpa_state != WPA_COMPLETED &&
        ...
        if (!is_zero_ether_addr(wpa_s->go_dev_addr))
            return wpa_s;
...
#endif
//[BUGFIX]-Add-END by TCTNB.Fan.He
